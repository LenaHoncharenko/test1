...
function wait(locator, time) {
	let button = driver.wait( until.elementLocated( locator ), 
	time = 20000
	);
	button.click( );
}

wait( By.xpath( '//div/input[ @data-type = "person" ]' ) );
...
wait( By.xpath( '//li/a[@href = "/newOrder/index" ]' ) );
wait( By.id( 'SenderSelectButton' ) );

function search(locator, value) {
	let input = driver.wait( until.elementLocated( locator ),
		time = 20000
		);
    input.sendKeys(value);
}

search( By.id( 'filter_journal_cities' ) , 'Полтава' );
search( By.id( 'filter_journal_address' ) , 'Ковпака' );
