...
waitAndClick( By.xpath( '//li/a[@href = "/newOrder/index" ]' ) )
	.then( waitAndClick( By.id( 'SenderSelectButton' ) ) )
	...
	.then( waitAndClick( By.xpath( '//li[@class = "browser_element_main active"]/a[1][ contains(text( ), "Суми") ]' ) ) )
	.then( waitAndSendKeys( By.id( 'filter_journal_address' ) , "Зеленко" ) ) 
	.then( waitAndClick( By.xpath( '//li[@class = "browser_element_main active"]/a[1][ contains(text( ), "Зеленко") ]' ) ) )
	.then( waitAndSendKeys( By.id( 'filter_journal_contacts' ), 'Гончаренко' ) ) 
	.then( waitAndClick( By.xpath( '//ul[@id = "contacts_ul"]/li[1][@data-lastname = "Гончаренко" ]' ) ) )
